...
# This script temporarily replaces the kernel in a remote bootc system
...
USER=${USER:-anatase}
NVIDIA_DIR="$(pwd)/../open-gpu-kernel-modules"
...
KIMAGE=$(make -s image_name)
install -m 0644 "$KIMAGE" ./out/vmlinuz
make -j $(expr $(nproc) - 2) modules_install INSTALL_MOD_PATH=./out/ -s

# Build and stage the NVIDIA open kernel modules alongside the in-tree modules.
# The existing rsync below then transports both sets of modules to the host.
if [ -f "$NVIDIA_DIR/Makefile" ]; then
    echo "Building NVIDIA modules from $NVIDIA_DIR"
    time PATH="/usr/lib/ccache/bin:$PATH" \
        CCACHE_DIR=$(pwd)/../cache CCACHE_FILECLONE=1 \
        CCACHE_MAXSIZE=4G CCACHE_IGNORECONFIG=1 \
        make -C "$NVIDIA_DIR" -s -j $(expr $(nproc) - 2) modules \
        SYSSRC="$(pwd)" SYSOUT="$(pwd)"

    echo "Installing NVIDIA modules to ./out"
    make -C "$NVIDIA_DIR" -s -j $(expr $(nproc) - 2) modules_install \
        SYSSRC="$(pwd)" SYSOUT="$(pwd)" INSTALL_MOD_PATH="$(pwd)/out"
else
