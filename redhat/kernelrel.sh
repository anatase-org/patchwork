...
(
        ...
        git -C ../kernel-anatase pull
        rm -f ./redhat/rpm/SRPMS/kernel-*.src.rpm || true
        make -C redhat dist-clean
        make -C redhat dist-srpm -j $(expr $(nproc) - 2) DIST=$dist BUILD=$relver IS_FEDORA=1
        ...
        # Lets ask first
        echo -n "anatase tag is $tag. Commit and push? [Y/n]"
        read answer
        if [ "$answer" == "n" ] || [ "$answer" == "N" ]; then
                echo "Aborting"
                exit 1
        fi
        git -C ../kernel-anatase commit -m "bump to $tag" || true
        git -C ../kernel-anatase tag -f $tag
        git -C ../kernel-anatase push origin $tag -f
        git tag -f $tag && git push origin $tag -f # only after we push the immutable tag
        git -C ../kernel-anatase push
        xdg-open "https://github.com/anatase-org/kernel-anatase/releases/new?tag=$tag"
)