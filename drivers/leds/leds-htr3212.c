...
static int htr3212_resume(struct device *dev)
{
	...
	ret = htr3212_init_regs(priv);
	if (ret)
		dev_err(dev, "Failed to reinitialize registers on resume (%pe)\n",
			ERR_PTR(ret));

	return ret;
}

static DEFINE_SIMPLE_DEV_PM_OPS(htr3212_pm_ops, htr3212_suspend,
				htr3212_resume);
